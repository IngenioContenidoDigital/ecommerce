              ...
              if (existShipping.length === 0) {
                ...
                if(city && oexists.length === 0){
                  let user = await User.findOrCreate({emailAddress:order.buyer.email},{
                    ...
                    dniType:'CC',
                    dni: order.buyer.billing_info && order.buyer['billing_info']['doc_number'] ? order.buyer['billing_info']['doc_number'].toString() : '',
                  ...
                  if((await CartProduct.count({cart:cart.id}))>0){
                    ...
                    await Order.updateOne({id:corders[0].id}).set({createdAt:parseInt(moment(order['date_created']).valueOf()),tracking: shipping['tracking_method'] ? shipping.id : ''});
                  } else {
