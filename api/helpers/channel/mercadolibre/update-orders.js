module.exports = {
  ...
  fn: async function (inputs, exits) {
    ...
    let integration = await sails.helpers.channel.mercadolibre.sign(order.seller);
    let result = await sails.helpers.channel.mercadolibre.findUser(integration.secret).catch(err =>{return exits.error(err.message);});

    if(result.id){
      if(order.tracking){
        let shipping = await sails.helpers.channel.mercadolibre.findShipments(integration.secret, order.tracking).catch(err=>{
