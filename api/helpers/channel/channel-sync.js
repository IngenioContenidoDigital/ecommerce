  ...
  fn: async function (inputs) {
    ...
    if (productChannels.length > 0) {
      for (const item of productChannels) {
        ...
        if (integration.channel.name === 'dafiti' && item.status && item.delete === false) {
        ...
        if (integration.channel.name === 'mercadolibre' && item.status && item.channelid !== '' && item.delete === false) {
        ...
        if (integration.channel.name === 'mercadolibremx' && item.status && item.channelid !== '' && item.delete === false) {
          let mlstatus = item.status ? 'active' : 'paused';
        ...
        if (integration.channel.name === 'linio' && item.status && item.delete === false) {
        ...
        if (integration.channel.name === 'liniomx' && item.status && item.delete === false) {
        ...
        if (integration.channel.name === 'coppel' && item.status && item.delete === false) {
        ...
        if (integration.channel.name === 'walmart' && item.status && item.delete === false)  {
          let axios = require('axios');
