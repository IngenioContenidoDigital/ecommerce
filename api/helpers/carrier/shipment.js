  ...
  fn: async function (inputs, exits) {
    ...
    if(order.channel==='muybacano'){
      ...
      let body = await sails.helpers.channel.muybacano.invoice(order,'invoice');
      if(body){
        options = {
          method: 'post',
          url: `${integration.channel.endpoint}/api/oms/pub/orders/${order.channelref}/invoice`,
          headers: {
      ...
      soap.createClient(url, options, (err, client) =>{
        ...
        method(requestArgs, async (err, result)=>{
          ...
          await Order.updateOne({id:inputs.order}).set({tracking:result.return.codigo_remision.$value});
          await sails.helpers.carrier.costs(result.return.codigo_remision.$value);
          body = await sails.helpers.channel.muybacano.invoice(order,'tracking');
          body.trackingUrl = result.return.url_terceros.$value;
          body.trackingNumber = result.return.codigo_remision.$value;
