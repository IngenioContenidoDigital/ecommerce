...
module.exports = {
  ...
  generateReportSeller:async (req, res) =>{
    ...
    try {
      ...
      doc.text('Devolución Retención por servicios', 15, 266);
      doc.text(`$ ${Math.round(data.devRteFteCommission).toLocaleString('es-CO')}`, 160, 266);
      doc.text('Retención de Ica 9,66/1000', 15, 274);
      doc.text(`$ ${Math.round(data.totalRetIca).toLocaleString('es-CO')}`, 160, 274);
      doc.text('Devolucion de Retención de Ica', 15, 282);
      doc.text(`$ ${Math.round(data.devRteIca).toLocaleString('es-CO')}`, 160, 282);
